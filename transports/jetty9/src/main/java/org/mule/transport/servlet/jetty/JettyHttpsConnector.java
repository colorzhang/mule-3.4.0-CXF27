...
public class JettyHttpsConnector extends JettyHttpConnector implements TlsDirectKeyStore, TlsIndirectKeyStore, TlsDirectTrustStore, TlsProtocolHandler
{
    ...
    @Override
    protected ServerConnector createJettyConnector()
    {
        ...
        if (tls.getKeyStore() != null) sslContextFactory.setKeyStorePath(tls.getKeyStore());
        if (tls.getKeyPassword() != null) sslContextFactory.setKeyStorePassword(tls.getKeyPassword());
        if (tls.getKeyStoreType() != null) sslContextFactory.setKeyStoreType(tls.getKeyStoreType());
        if (tls.getKeyManagerAlgorithm() != null) sslContextFactory.setSslKeyManagerFactoryAlgorithm(tls.getKeyManagerAlgorithm());
        if (tls.getProvider() != null) sslContextFactory.setProvider(tls.getProvider().getName());
        if (tls.getTrustStorePassword() != null) sslContextFactory.setTrustStorePassword(tls.getTrustStorePassword());
        if (tls.getTrustStore() != null) sslContextFactory.setTrustStorePath(tls.getTrustStore());
        if (tls.getTrustStoreType() != null) sslContextFactory.setTrustStoreType(tls.getTrustStoreType());
        if (tls.getTrustManagerAlgorithm() != null) sslContextFactory.setTrustManagerFactoryAlgorithm(tls.getTrustManagerAlgorithm());
        
	ServerConnector sc = new ServerConnector(httpServer, sslContextFactory);
	return sc;
    }
}
